...
meta:
  ...
  description: A guide to understanding the es6 generators with simple examples.
  "og:title": A Simple Guide to Understanding ES6 Generators
  "og:description": A guide to understanding the es6 generators with simple examples.
---
...
![](https://cdn-images-1.medium.com/max/800/1*bwQSEHpbaNHte95IW2kTCw.jpeg)_Generator Turbine_
...
> I recommend the following link for those of you who are new to iterators, [Guide to Iterators](https://medium.com/r/?url=https%3A%2F%2Fcodeburst.io%2Fa-simple-guide-to-es6-iterators-in-javascript-with-examples-189d052c3d8e).
...
Syntactically they are identified with a star. Eg: _function \*X or function\* X_.
...
![](https://cdn-images-1.medium.com/max/800/1*YnOJNuFe-r9T7pO47mVYaw.png)_Basic generator function_
...
![](https://cdn-images-1.medium.com/max/800/1*0pLkX6yrbV2r6_pZ10AIvQ.png)_Life-cycle of a Generator Function_
...
![](https://cdn-images-1.medium.com/max/800/1*zdJQlUaqIiD3eV0j0QzrZA.png)_Assigning yield to a variable_
...
![](https://cdn-images-1.medium.com/max/800/1*aYCKrAkgSyfEeN9cswZzbA.png)_Passing arguments to the next()_

### Explanation:

1.  When we call the **_first next(20)_**, every line of code till the first yield is printed. As we do not have any previous yield expression this value 20 is discarded. In the output we get yield value as i10, which is 100 here. Also the state of the execution stops with first yield and the **_const j_** is not yet set.
...
![](https://cdn-images-1.medium.com/max/800/1*Y6pwTwJ7stPZzAeCKBfv4Q.png)_Yield as an argument of a function_
...
![](https://cdn-images-1.medium.com/max/800/1*zXpsq-hlqla3z3mZGWyTJw.png)_Yield calling a function_
...
![](https://cdn-images-1.medium.com/max/800/1*100c_wLxJHmcKtjZAYwJzw.png)_Yield with promises_

The apiCall returns the promises as the yield value, when resolved after 2 seconds prints the value we need.

## Yield

So far we have been looking at the use cases for yield expression, now we are going to look at another expression called **_yield_**_._ **_Yield_** when used inside a generator function delegates another generator function. Simply put, it synchronously completes the generator function in its expression before moving on to the next line.

Let’s look at the code and the explanation below to understand better. This code is from the MDN web docs.

![](https://cdn-images-1.medium.com/max/800/1*eMlOmBoi2XGCE3qwUIj3qA.png)_Basic yield_
...
> This will go on to finish the generator run. However, there is one distinct ability of the **_yield_** which you should keep in mind while using **_return_**, in the next section.
...
![](https://cdn-images-1.medium.com/max/800/1*HxJtIuXhBnOMAK0cwVElsQ.png)_Yield with return_
...
There is one more interesting yield property worth-mentioning, similar to the return value, **_yield_** can also iterate over iterable objects like Array, String and Map.

Let’s look at how it works in real-time.

![](https://cdn-images-1.medium.com/max/800/1*u6RQVCQBCqw5UsF3Kger1w.png)_Yield over built-in iterables_
...
![](https://cdn-images-1.medium.com/max/800/1*dDYt_xElLC7wjUDN7HfDJg.png)_Yield with for…of_
